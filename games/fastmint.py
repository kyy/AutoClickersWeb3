...
TELEGRAM_URL = "https://web.telegram.org/k/#@fastmintapp_bot"
...
async def run(playwright: Playwright):
    ...
    try:
        await page.get_by_role("button", name="Claim").tap(force=True)  # claim
    except:
        pass

    time.sleep(1)
    try:
        await page.get_by_role("button").filter(has_text="Claim").tap(force=True)  # claim
    except:
        pass

    time.sleep(1)
    try:
        await page.get_by_role("button", name="Start farming").tap(force=True)  # start
    except:
        pass
